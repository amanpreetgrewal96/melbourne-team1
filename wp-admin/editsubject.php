                  ...
                  <div class="col-lg-12">
                      ...
                      <?php
                        if(isset($_POST['submit']))
                        {
                            ...
                            {
                                $query_check=mysqli_query($connection,"select * from rms_subject where class='$name' && id!='".$_SESSION['courseid']."'");
                                if(mysqli_num_rows($query_check)<=0)
                                {
                                   $query=mysqli_query($connection,"update rms_subject set class='$name',subjects='$duration' where id='".$_SESSION['courseid']."'");
                                    if(mysqli_affected_rows($connection))
                                    {
                                        echo '<div class="alert alert-success">Subjects are Updated Successfully.</div>';
                                   header('Refresh: 2;url=subjectup.php');
                                    }
                                    else
                                    {
                         ...
                         <div class="panel-body">
                              <form class="form-horizontal" role="form" method="post">
                                  ...
                                  if($_SESSION['courseid'])
                                  {
                                    ...
                                    $query1=mysqli_query($connection,"select * from rms_subject where id='$courseid'");
                                    if(mysqli_num_rows($query1)>0)
                                    {
                                        $row1=mysqli_fetch_array($query1);
                                        $name1=$row1['class'];
                                        $duration1=$row1['subjects'];
                                    }
                                  }
                                  ...
                                  <div class="form-group">
                                      <label for="Duration" class="col-lg-2 col-sm-2 control-label">subjects</label>
                                      <div class="col-lg-10">
